...
//
// /**
//  * @param table Name of the table to select. Auto-completion gives the list of choices. It
// should
...
// //  * unmounted. To access subscription data, use selector `selectApollo` define in the same
// file. //  * This is a replacement for useSubscription that don't trigger component re-render
// when //  * loading/data/error change. Selectors are responsible for triggering re-renders. //  *
// //  * Only use this method when you're not happy with the existing useSubscription and would
// like //  * to use redux selectors instead to get loading and errors. //  */ // export function
// useSimpleSubscription<TData = any, //   TVariables = OperationVariables>(subscription:
// DocumentNode, //                                    options?: SubscriptionHookOptions<TData,
// TVariables>): void { //   useEffect(() => { //     const client = getGqlClient(); //     const
// sub = client.subscribe<TData, TVariables>( //       { query: subscription, variables:
// options?.variables, fetchPolicy: options?.fetchPolicy }) //       .subscribe(noOp); //
// return () => sub.unsubscribe(); //   }, []); // } // // function noOp() { // }

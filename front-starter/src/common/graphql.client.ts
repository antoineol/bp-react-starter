...
//         if (code === 'invalid-jwt'
//           || (code === 'invalid-headers' && err.message.includes('Missing Authorization
// header'))) { return reauthAndRetry(operation, forward); } } } if (networkError) { const code =
// (networkError as any)?.extensions?.code; console.error('[Network error]:', networkError); if
// (code === 'start-failed') { return reauthAndRetry(operation, forward); } } });  const authLink =
// setContext(async (_, { headers }) => ({ headers: await addJwtToHeaders(headers) }));  wsLink =
// new WebSocketLink({ uri: `${env.hasuraWs}/graphql`, options: { reconnect: true, lazy: true, //
// reconnectionAttempts: 999, // Infinite if not provided connectionParams: async () => ({ headers:
